    ...
    while game == True:
        ...
        for event in pygame.event.get():
            ...
            if event.type== KEYDOWN:
                ...
                if event.key== ord('a'):
                    ...
                    cows.append([cowX, cowY, cowX, cowY, 5])
                    print("Cow")
        ...
        while i < len(cows):
            ...
            if abs(cows[i][0]-cows[i][2]) > abs(cows[i][1]-cows[i][3]): # move the cow to it's target
                if(cows[i][0] > cows[i][2]):
                    cows[i][0] -= 1
                elif(cows[i][0] < cows[i][2]):
                    cows[i][0] += 1
            elif abs(cows[i][0]-cows[i][2]) < abs(cows[i][1]-cows[i][3]):
                if(cows[i][1] > cows[i][3]):
                    cows[i][1] -= 1
                elif(cows[i][1] < cows[i][3]):
                    cows[i][1] += 1
            ...
            if(gridobjects[cows[i][0]][cows[i][1]] == "tall grass"): #cow eats grass
                gridobjects[cows[i][0]][cows[i][1]] = "short grass"
                cows[i][4] += 1
            #else: #no grass :,(
            ...
            grassY = -1
            z=0
            while z < 7:  # SEARCH FOR GRASS
                zz=0
                while zz < 7:
                    try:
                        if gridobjects[z+(cows[i][1])-3][zz+(cows[i][1])-3] == "tall grass":
                            grassysmell= sqrt(pow(cows[i][0]-(z+cows[i][0]-3),(2)) + pow(cows[i][1]-(zz+cows[i][1]-3),(2)))
                            if(grassyhigh == -1 or grassyhigh < grassysmell):
                                grassyhigh = grassysmell
                                grassyX = (z+cows[i][0]-3)
                                grassyY = (zz+cows[i][1]-3)
                        #if(gridobjects[z+cows[i][1]-3][zz+cows[i][1]-3]] == "fire"):
                        #    grassyhigh = -2
                        #    grassyX = randInt()
                    ...
                    zz+=1
                z+=1
            cows[i][2] = grassX # set new destination for cow (closest grass)
            cows[i][3] = grassY
